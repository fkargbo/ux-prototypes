...
import { buildPrototypeHref } from '../v1PerspectiveUrl';
import { PLANS_LIST_PATH, TROUBLESHOOTING_PLANS_LIST_PATH } from '../v1PerspectiveUrl';
...
export function resolvePlanDomainAnnotations(plan: PlanRow): PlanDomainAnnotations {
  if (isObservabilityPlan(plan)) {
    return {
      ...
      detailPath: `/v1/ai-hub/observe/troubleshooting-plans/${encodeURIComponent(plan.id)}`,
  ...
  if (isSecurityPlan(plan)) {
    ...
    return {
      sourceDomain: 'acs',
      detailPath: `/v1/ai-hub/observe/acs-plans/${encodeURIComponent(slug)}`,
  ...
  return {
    ...
    detailPath: `/v1/ai-hub/observe/plans/${encodeURIComponent(slug)}/remediation`,
    listPath: PLANS_LIST_PATH,
    listBreadcrumbLabel: 'Plans',
  };
}
