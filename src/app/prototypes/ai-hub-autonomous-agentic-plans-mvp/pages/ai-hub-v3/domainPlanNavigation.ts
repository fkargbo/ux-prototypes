...
import { PLANS_LIST_PATH, TROUBLESHOOTING_PLANS_LIST_PATH } from '../../prototypePerspectiveUrl';
...
export function resolvePlanDomainAnnotations(plan: PlanRow): PlanDomainAnnotations {
  if (isObservabilityPlan(plan)) {
    return {
      ...
      detailPath: `/core/observe/troubleshooting-plans/${encodeURIComponent(plan.id)}`,
  ...
  if (isSecurityPlan(plan)) {
    ...
    return {
      sourceDomain: 'acs',
      detailPath: `/core/observe/ai-hub/acs-plans/${encodeURIComponent(slug)}`,
  ...
  return {
    ...
    detailPath: `/core/observe/ai-hub/plans/${encodeURIComponent(slug)}/remediation`,
    listPath: PLANS_LIST_PATH,
    listBreadcrumbLabel: 'Plans',
  };
}
