...
/** Mock fleet-wide counts for the AI Hub inventory bar (not modeled per cluster). */
...
/** Aggregates for the fleet inventory strip (clusters/nodes from mock clusters; rest from fleet constants + alerts list). */
...
/**
 * Per-cluster inventory for Core platforms hub bar — namespaces/workloads scale with node count for plausible demo totals.
 */
