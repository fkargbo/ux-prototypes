...
import {
  ...
  isNewAlertInvestigationPlanVisible,
} from './alertInvestigationPlans';
import {
  GITOPS_INVESTIGATION_DRAWER_DATA,
  GITOPS_INVESTIGATION_PLAN_IDENTITY,
  GITOPS_INVESTIGATION_PLANS,
  isGitOpsInvestigationPlanVisible,
} from './gitOpsInvestigationPlans';
import {
  PIPELINES_INVESTIGATION_DRAWER_DATA,
  PIPELINES_INVESTIGATION_PLAN_IDENTITY,
  PIPELINES_INVESTIGATION_PLANS,
  isPipelinesInvestigationPlanVisible,
} from './pipelinesInvestigationPlans';
...
const PLAN_SORT_SCORES: Record<string, number> = {
  ...
  'inv-gitops-agentit': 79,
  'inv-gitops-pulse-agent': 77,
  'inv-gitops-pulse-ui': 75,
};
...
const ALL_PLANS: RawPlanRow[] = [
  ...
  ...GITOPS_INVESTIGATION_PLANS.map((plan) => ({
    ...plan,
    drawerTargets: ['prod-east-2'],
  })),
  ...PIPELINES_INVESTIGATION_PLANS.map((plan) => ({
    ...plan,
    drawerTargets: ['prod-east-2'],
  })),
];
...
export function buildPlansForPerspective(
  ...
  return [...combined]
    ...
    .filter((plan) => {
      ...
      return (
        isNewAlertInvestigationPlanVisible(row) &&
        isGitOpsInvestigationPlanVisible(row) &&
        isPipelinesInvestigationPlanVisible(row)
      );
    }) as PlanRow[];
}
