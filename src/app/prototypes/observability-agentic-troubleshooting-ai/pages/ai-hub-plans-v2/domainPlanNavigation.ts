...
export type PlanSourceDomain = 'observability' | 'acs' | 'cluster-update' | 'gitops' | 'pipelines';
...
function isGitOpsPlan(plan: PlanRow): boolean {
  return plan.triggerDomain === 'GitOps';
}

function isPipelinesPlan(plan: PlanRow): boolean {
  return plan.triggerDomain === 'Pipelines';
}
...
export function resolvePlanDomainAnnotations(plan: PlanRow): PlanDomainAnnotations {
  ...
  if (isPipelinesPlan(plan)) {
    return {
      sourceDomain: 'pipelines',
      detailPath: `/v2/ai-hub/agentic-runs/runs/${encodeURIComponent(plan.id)}`,
      listPath: PLANS_LIST_PATH,
      listBreadcrumbLabel: 'Agentic runs',
    };
