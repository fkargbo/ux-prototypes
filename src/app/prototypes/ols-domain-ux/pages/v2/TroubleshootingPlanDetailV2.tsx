...
export type PlanDetailListContext = 'agentic-runs' | 'recommendation-hub';

export const TroubleshootingPlanDetailV2: React.FC<{ listContext?: PlanDetailListContext }> = ({
  listContext = 'agentic-runs',
}) => {
  ...
  const navigateBackToPlans = useCallback(() => {
    ...
    const listPath =
      listContext === 'recommendation-hub'
        ? RECOMMENDATION_HUB_LIST_PATH
        : (planDomain?.listPath ?? TROUBLESHOOTING_PLANS_LIST_PATH);
    navigate(buildPrototypeHref(listPath, key));
  }, [activePerspective, drillPerspectiveKey, listContext, navigate, planDomain?.listPath, setPerspectiveByKey]);
      ...
      <div className="template-page-breadcrumb">
        <Breadcrumb>
          <BreadcrumbItem component="button" onClick={navigateBackToPlans}>
            {listContext === 'recommendation-hub' ? 'Recommendation hub' : 'Agentic runs'}
          </BreadcrumbItem>
          <BreadcrumbItem isActive>
            {listContext === 'recommendation-hub' ? 'Recommendation details' : 'Agentic run details'}
          </BreadcrumbItem>
        </Breadcrumb>
      </div>
