...
const MultiClusterAlertingDashboard: React.FunctionComponent = () => {
  ...
  // Create Alert Rule Wizard state
  const [isCreateAlertRuleWizardOpen, setIsCreateAlertRuleWizardOpen] = React.useState(false);
  ...
  const [alertRuleComponent, setAlertRuleComponent] = React.useState<AlertComponent | ''>('');
  ...
  const [alertRuleEnvironmentFilter, setAlertRuleEnvironmentFilter] = React.useState<string[]>([]);
  const [alertRuleRegionFilter, setAlertRuleRegionFilter] = React.useState<string[]>([]);
...
spec:
  ...
  return (
    <div className="alerting-page-container" style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
                ...
                <Card>
                  <CardHeader>
                    <Flex justifyContent={{ default: 'justifyContentSpaceBetween' }} alignItems={{ default: 'alignItemsCenter' }}>
                      ...
                      <FlexItem>
                        <Button variant="primary" icon={<PlusIcon />} onClick={() => setIsCreateAlertRuleWizardOpen(true)}>Create Alert Rule</Button>
                      </FlexItem>
                    </Flex>
                  </CardHeader>
                  <CardBody>
      ...
      {/* Create Alert Rule Wizard Modal */}
      <Modal
        isOpen={isCreateAlertRuleWizardOpen}
        onClose={() => setIsCreateAlertRuleWizardOpen(false)}
        aria-labelledby="create-alert-rule-wizard-title"
        variant="large"
      >
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--pf-t--global--border--color--default)' }}>
            ...
            <div style={{ marginTop: '16px' }}>
              <Flex alignItems={{ default: 'alignItemsCenter' }} gap={{ default: 'gapMd' }}>
                <FlexItem>
                  <Title headingLevel="h1" size="xl">Create alert rules</Title>
                </FlexItem>
                <FlexItem>
                  <Switch
                    id="yaml-mode-switch"
            ...
            >
                      ...
                      <FlexItem flex={{ default: 'flex_1' }}>
                        ...
                        >
                          <Select
                            ...
                            onSelect={(_, value) => { setAlertRuleComponent(value as AlertComponent); setIsAlertRuleComponentOpen(false); }}
                            selected={alertRuleComponent || undefined}
                            toggle={(toggleRef: React.Ref<MenuToggleElement>) => (
                              <MenuToggle ref={toggleRef} onClick={() => setIsAlertRuleComponentOpen(!isAlertRuleComponentOpen)} isExpanded={isAlertRuleComponentOpen} style={{ width: '100%' }}>
                                {alertRuleComponent || 'Select a component'}
                              </MenuToggle>
                            )}
                          >
                            <SelectList>
                              <SelectOption value="kube-apiserver">kube-apiserver</SelectOption>
                              <SelectOption value="Storage">Storage</SelectOption>
                              <SelectOption value="Network">Network</SelectOption>
                              <SelectOption value="etcd">etcd</SelectOption>
                              <SelectOption value="Scheduler">Scheduler</SelectOption>
                              <SelectOption value="Controller">Controller</SelectOption>
                              <SelectOption value="Workload">Workload</SelectOption>
                              <SelectOption value="Pod">Pod</SelectOption>
                              <SelectOption value="Quota">Quota</SelectOption>
                            </SelectList>
                          </Select>
                        </FormGroup>
                      </FlexItem>
                  ...
                  <Form>
                    ...
                    <FormGroup fieldId="alert-rule-runbook">
                      <Checkbox
                        ...
                        description="Runbooks provide clear, step-by-step instructions for completing a specific task. Add a URL to your alert rule that will be shown when the alert is firing."
                      />
                      {alertRuleAddRunbook && (
                        <FormGroup
                          label={
                    ...
                    <div style={{ marginTop: '24px' }}>
                      ...
                      <FormGroup fieldId="alert-rule-route-by-label">
                        <Checkbox
                          ...
                          description="Automatically sends alerts to the appropriate receivers using labels such as cluster, region, and owner_team."
                        />
                  ...
                  <Form>
                    ...
                    {!alertRuleTargetAllClusters && (
                      <div style={{ marginTop: '16px', padding: '16px', backgroundColor: 'var(--pf-t--global--background--color--secondary--default)', borderRadius: '8px' }}>
                            ...
                            <Flex alignItems={{ default: 'alignItemsCenter' }} gap={{ default: 'gapSm' }}>
                              <FlexItem>
                                <Checkbox 
                                  ...
                                  isChecked={alertRuleSelectedClusters.length === mockClusters.length}
                                  onChange={(_, checked) => {
                                    if (checked) {
                                      setAlertRuleSelectedClusters(mockClusters.map(c => c.name));
                                    } else {
                        ...
                        {/* Filter chips */}
                        <Flex gap={{ default: 'gapSm' }} alignItems={{ default: 'alignItemsCenter' }} style={{ marginBottom: '16px' }}>
                          <FlexItem>
                            <Label color="grey">
                              Environment
                              {alertRuleEnvironmentFilter.map((env, idx) => (
                                <Label key={idx} color="blue" isCompact onClose={() => setAlertRuleEnvironmentFilter(alertRuleEnvironmentFilter.filter(e => e !== env))} style={{ marginLeft: '4px' }}>
                                  {env}
                                </Label>
                              ))}
                              <Button variant="plain" size="sm" onClick={() => setAlertRuleEnvironmentFilter([])}>
                                <TimesIcon />
                              </Button>
                            </Label>
                          </FlexItem>
                          <FlexItem>
                            <Label color="grey">
                              Region
                              {alertRuleRegionFilter.map((reg, idx) => (
                                <Label key={idx} color="blue" isCompact onClose={() => setAlertRuleRegionFilter(alertRuleRegionFilter.filter(r => r !== reg))} style={{ marginLeft: '4px' }}>
                                  {reg}
                                </Label>
                              ))}
                              {alertRuleRegionFilter.length > 3 && <Badge isRead>{alertRuleRegionFilter.length - 3} more</Badge>}
                              <Button variant="plain" size="sm" onClick={() => setAlertRuleRegionFilter([])}>
                                <TimesIcon />
                              </Button>
                            </Label>
                          </FlexItem>
                        ...
                        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                          {mockClusters
                            .filter(cluster => 
                              !alertRuleClusterSearch || 
                              cluster.name.toLowerCase().includes(alertRuleClusterSearch.toLowerCase())
                            )
                            .slice(0, 10)
                            .map((cluster) => (
                            <div 
                              ...
                              <Flex alignItems={{ default: 'alignItemsFlexStart' }} gap={{ default: 'gapMd' }}>
                                <FlexItem>
                                  <Button 
                                    ...
                                    size="sm"
                                    onClick={() => {
                                      if (alertRuleExpandedClusters.includes(cluster.name)) {
                                        setAlertRuleExpandedClusters(alertRuleExpandedClusters.filter(c => c !== cluster.name));
                                      } else {
                                        setAlertRuleExpandedClusters([...alertRuleExpandedClusters, cluster.name]);
                                      }
                                    }}
                                  >
                                    {alertRuleExpandedClusters.includes(cluster.name) ? <AngleDownIcon /> : <AngleRightIcon />}
                                  </Button>
                                </FlexItem>
                                <FlexItem>
                                  <Checkbox
                                ...
                                <FlexItem flex={{ default: 'flex_1' }}>
                                  <Flex direction={{ default: 'column' }} gap={{ default: 'gapXs' }}>
                                    <FlexItem>
                                      <Flex alignItems={{ default: 'alignItemsCenter' }} gap={{ default: 'gapSm' }}>
                                        <FlexItem>
                                          {getClusterAlertStatus(cluster) === 'critical' && <ExclamationTriangleIcon color="var(--pf-t--global--color--status--warning--default)" />}
                                          {getClusterAlertStatus(cluster) === 'warning' && <ExclamationTriangleIcon color="var(--pf-t--global--color--status--warning--default)" />}
                                          {getClusterAlertStatus(cluster) === 'healthy' && <CheckCircleIcon color="var(--pf-t--global--color--status--success--default)" />}
                                        </FlexItem>
                                    ...
                                    <FlexItem>
                                      <Content component="small" style={{ color: 'var(--pf-t--global--text--color--subtle)' }}>
                                        {cluster.alerts.length > 0 ? `High turnover of alerts.` : 'No active alerts.'}
                                      </Content>
                                    </FlexItem>
                                    <FlexItem>
                                      <Flex gap={{ default: 'gapMd' }} flexWrap={{ default: 'wrap' }}>
                                        <FlexItem>
                                          <Content component="small"><strong>Environment:</strong> {cluster.labels.environment || 'Production'}</Content>
                                        </FlexItem>
                                        <FlexItem>
                                          <Content component="small"><strong>Region / Availability Zone:</strong> {cluster.region}</Content>
                                        </FlexItem>
                                        <FlexItem>
                                          <Content component="small"><strong>Version:</strong> OCP {cluster.labels.version || '4.14'}</Content>
                                        </FlexItem>
                                        <FlexItem>
                                          <Button variant="link" isInline size="sm">{Object.keys(cluster.labels).length} more labels</Button>
                                        </FlexItem>
                                      </Flex>
                                    </FlexItem>
                                  </Flex>
              ...
              <WizardStep
                ...
                footer={{
                  ...
                  onNext: () => {
                    setIsCreateAlertRuleWizardOpen(false);
                    addToast('Alert rule created successfully', 'success');
                    // Reset form
                    setAlertRuleName('');
                    setAlertRuleExpression('');
                    setAlertRuleSeverity('Critical');
                    setAlertRuleLabels([]);
                    setAlertRuleSelectedClusters([]);
                  }
                }}
          ...
          {/* Footer with Download YAML option on last step */}
          <div style={{ padding: '16px 24px', borderTop: '1px solid var(--pf-t--global--border--color--default)', display: 'flex', gap: '16px' }}>
